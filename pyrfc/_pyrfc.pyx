...
cdef class Connection:
    ...
    cdef public bint return_import_params
    ...
    def __init__(self, config=None, **params):
        ...
        self.return_import_params = config.get('return_import_params', False)
    ...
    def get_connection_attributes(self):
        ...
        result = wrapConnectionAttributes(attributes, True) # rstrip=True
        result.update({
    ...
    def call(self, func_name, **params):
        ...
        :return: Dictionary with all EXPORT, CHANGING, and TABLE parameters.
              The IMPORT parameters are also given, if :attr:`Connection.return_import_params`
              is set to ``True``.
        ...
        try: # now we have a function module
            ...
            if rc != RFC_OK:
                self._error(&errorInfo)
            if self.return_import_params is True:
                return wrapResult(funcDesc, funcCont, <RFC_DIRECTION> 0, self.rstrip)
            else:
                return wrapResult(funcDesc, funcCont, RFC_IMPORT, self.rstrip)
        finally:
    ...
    def _create_and_submit_unit(self, unit_id, calls, queue_names=None, attributes=None):
        ...
        if len(queue_names) == 0:
            ...
            queueNameCount = len(queue_names)
            queueNames = <SAP_UC**> mallocU(queueNameCount * sizeof(SAP_UC*))
...
cdef RFC_RC genericRequestHandler(RFC_CONNECTION_HANDLE rfcHandle, RFC_FUNCTION_HANDLE funcHandle, RFC_ERROR_INFO* serverErrorInfo):
    ...
    try:
        ...
        if rc != RFC_OK:
            ...
            conn_attr = wrapConnectionAttributes(attributes, True) # rstrip=True
...
cdef wrapConnectionAttributes(RFC_ATTRIBUTES attributes, rstrip=False):
    return {
...
cdef wrapResult(RFC_FUNCTION_DESC_HANDLE funcDesc, RFC_FUNCTION_HANDLE container, RFC_DIRECTION filter_parameter_direction, rstrip=False):
    """
    ...
    :param rstrip: right strip strings.
    :return:
    """
    cdef RFC_RC rc
    cdef RFC_ERROR_INFO errorInfo
    ...
    for i in range(paramCount):
        ...
        if paramDesc.direction != filter_parameter_direction:
            result[wrapString(paramDesc.name)] = wrapVariable(paramDesc.type, container, paramDesc.name, paramDesc.nucLength, paramDesc.typeDescHandle, rstrip)
    return result
...
cdef wrapStructure(RFC_TYPE_DESC_HANDLE typeDesc, RFC_STRUCTURE_HANDLE container, rstrip=False):
    cdef RFC_RC rc
    cdef RFC_ERROR_INFO errorInfo
    cdef unsigned i, fieldCount
    cdef RFC_FIELD_DESC fieldDesc
    ...
    for i in range(fieldCount):
        ...
        result[wrapString(fieldDesc.name)] = wrapVariable(fieldDesc.type, container, fieldDesc.name, fieldDesc.nucLength, fieldDesc.typeDescHandle, rstrip)
    return result
...
cdef wrapTable(RFC_TYPE_DESC_HANDLE typeDesc, RFC_TABLE_HANDLE container, rstrip=False):
    cdef RFC_RC rc
    cdef RFC_ERROR_INFO errorInfo
    cdef unsigned lines
    ...
    for i in range(lines): #TODO: lazy?
        ...
        result.append(wrapStructure(typeDesc, container, rstrip))
    return result

cdef wrapVariable(RFCTYPE typ, RFC_FUNCTION_HANDLE container, SAP_UC* cName, unsigned cLen, RFC_TYPE_DESC_HANDLE typeDesc, rstrip=False):
    cdef RFC_RC rc
    cdef RFC_ERROR_INFO errorInfo
    ...
    if typ == RFCTYPE_STRUCTURE:
        ...
        if rc != RFC_OK:
            raise wrapError(&errorInfo)
        return wrapStructure(typeDesc, structure, rstrip)
    elif typ == RFCTYPE_TABLE:
        rc = RfcGetTable(container, cName, &table, &errorInfo)
        if rc != RFC_OK:
            raise wrapError(&errorInfo)
        return wrapTable(typeDesc, table, rstrip)
    elif typ == RFCTYPE_CHAR:
        ...
        try:
            ...
            if rc != RFC_OK:
                raise wrapError(&errorInfo)
            return wrapString(charValue, cLen, rstrip)
        finally:
            free(charValue)
    elif typ == RFCTYPE_STRING:
        ...
        try:
            ...
            if rc != RFC_OK:
                raise wrapError(&errorInfo)
            return Decimal(wrapString(stringValue, -1, rstrip))
        finally:
            free(stringValue)
    elif typ == RFCTYPE_FLOAT:
